...
census_data <- get_census(dataset='CA16', regions=regions, vectors=vectors, level='CT',geo_format="sf")
...
prep_data <- function(geo){
  labels <- label_vectors(geo)
  geo <- geo %>% detail_labels %>% replace(is.na(.), 0)
  attr(geo,'dot_labels') <- labels
  return(geo)
}
```
...
data_ct <- get_census(geo_format='sf',labels='short',dataset=dataset, regions=regions, vectors=vectors,level="CT") %>% prep_data
data_da <- get_census(geo_format='sf',labels='short',dataset=dataset, regions=regions, vectors=vectors,level="DA") %>% prep_data
data_db <- get_census(geo_format='sf',labels='short',dataset=dataset, regions=regions, vectors=vectors,level="DB") 
```
## Mapping
...
categories=attr(data_ct,'dot_labels')$Detail
...
```{r, fig.width=12}
data_da <- dot_density.proportional_re_aggregate(data=data_da,parent_data=data_ct,geo_match=setNames("GeoUID","CT_UID"),categories=categories,base="Households")
data_db <- dot_density.proportional_re_aggregate(data=data_db,parent_data=data_da,geo_match=setNames("GeoUID","DA_UID"),categories=categories,base="Households")
